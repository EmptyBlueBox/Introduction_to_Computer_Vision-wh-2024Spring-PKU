...
if __name__ == "__main__":
    ...
    for i in range(50):
        ...
        # 反向传播
        loss_batch = -(gt_y * np.log(pred_y) + (1 - gt_y) * np.log(1 - pred_y))
        d_pred_y = -gt_y/pred_y + (1-gt_y)/(1-pred_y)
        d_output_layer_2 = d_pred_y * pred_y * (1-pred_y)
        d_MLP_layer_2 = output_layer_1_act.T.dot(d_output_layer_2)  # 16*1
        d_output_layer_1_act = d_output_layer_2.dot(MLP_layer_2.T)  # 10*16
        d_output_layer_1 = d_output_layer_1_act * output_layer_1_act * (1-output_layer_1_act)  # 10*16
        d_MLP_layer_1 = input_vector.T.dot(d_output_layer_1)  # 784*16

        # Backward : compute the gradient of paratmerters of layer1 (grad_layer_1) and layer2 (grad_layer_2)
        grad_layer_1 = d_MLP_layer_1
        grad_layer_2 = d_MLP_layer_2

        MLP_layer_1 -= lr * grad_layer_1
        MLP_layer_2 -= lr * grad_layer_2

    os.makedirs("results", exist_ok=True)
    np.savetxt("results/HW1_BP.txt", loss_list)
