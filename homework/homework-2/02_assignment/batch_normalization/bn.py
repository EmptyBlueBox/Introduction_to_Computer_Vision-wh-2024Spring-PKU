...
def bn_backward(dout, cache):
    ...
    dx = dx_hat/np.sqrt(sample_var+eps)+dsample_var*2*(x-sample_mean)/N+dsample_mean/N  # dy/dx = ∂y/∂x + ∂y/∂var * dvar/dx + ∂y/∂mean * dmean/dx
    # ----------------TODO------------------
...
if __name__ == "__main__":
    HW_dir = os.path.dirname(os.path.dirname(__file__))
    ...
    for i in range(50):
        ...
        # ---------------- TODO -------------------
        # update mean & var
        _, _, mean_sample, var_sample, _ = cache
        mean = rho*mean+(1-rho)*mean_sample
        var = rho*var+(1-rho)*var_sample
        # ---------------- TODO -------------------
